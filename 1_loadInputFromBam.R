...
loadInputFromBam=function(bamfile,
                   genomefile)
{
  algn=readBAM(bamfile)
  Genome=readGenome(genomefile)
  if (length(base::intersect(seqlevels(algn), names(Genome))) == length(Genome))
  {
...
readBAM <- function(bamfile)
{
  ...
  param <- Rsamtools::ScanBamParam(what = c("seq","strand"), tag = "XG")
