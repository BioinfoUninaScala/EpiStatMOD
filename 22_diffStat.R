...
diffStat <- function(intervals_list,
  ...
  res = tryCatch(
    {
      foreach::foreach(a = datablocks, .combine = rbind, .packages = c("magrittr", "dplyr"), .export = 'onefun') %dopar% {
...
onefun <- function(datalst, metadata, colsamples, statistic, groupcol, min.per.group, min.groups, reduce){
  ...
  filter <- datalst %>% dplyr::left_join(., metadata, by = c("sample" = colsamples)) %>%
    dplyr::group_by(id, .data[[groupcol]]) %>%
...
# onefun <- function(datalst, metadata, colsamples, statistic, groupcol,
#                    min.per.group, min.groups, reduce) {
#   
#   # metto tutto insieme
#   datalst <- datalst %>% dplyr::bind_rows()
#   
#   ## Filter regions (come prima, ma teniamo solo tibble-like)
#   filter <- datalst %>%
#     dplyr::left_join(metadata, by = c("sample" = colsamples)) %>%
#     dplyr::group_by(id, !!rlang::sym(groupcol)) %>%
#     dplyr::mutate(samples = dplyr::n_distinct(sample)) %>%
#     dplyr::filter(samples >= min.per.group) %>%
#     dplyr::group_by(id) %>%
#     dplyr::mutate(groups = dplyr::n_distinct(.data[[groupcol]])) %>%
#     dplyr::filter(groups >= min.groups & samples >= min.per.group) %>%
#     dplyr::ungroup() %>%
#     dplyr::select(id, sample, all_of(statistic), all_of(groupcol))
#   
#   if (nrow(filter) == 0L) {
#     return(filter)
#   }
#   
#   ## funzione helper per testare una singola regione
#   region_test <- function(df) {
#     df <- df %>%
#       dplyr::filter(!is.na(.data[[statistic]]),
#                     !is.na(.data[[groupcol]]))
#     
#     if (nrow(df) == 0L) {
#       return(tibble::tibble(p.value = NA_real_, type = NA_character_))
#     }
#     
#     g <- droplevels(factor(df[[groupcol]]))
#     ng <- nlevels(g)
#     
#     # se < 2 gruppi: impossibile testare
#     if (ng < 2L) {
#       return(tibble::tibble(p.value = NA_real_, type = NA_character_))
#     }
#     
...
#     
#     if (ng == 2L) {
#       # Wilcoxon solo se davvero 2 livelli
#       p <- tryCatch(
#         stats::wilcox.test(y ~ g)$p.value,
#         error = function(e) NA_real_
#       )
#       tibble::tibble(p.value = p, type = "wilcox.test")
#     } else {
#       # più di 2 gruppi → Kruskal
#       p <- tryCatch(
#         stats::kruskal.test(y ~ g)$p.value,
#         error = function(e) NA_real_
#       )
#       tibble::tibble(p.value = p, type = "kruskal.test")
#     }
#   }
#   
#   ## applico il test per ogni regione
#   res <- filter %>%
#     dplyr::group_by(id) %>%
#     tidyr::nest() %>%
#     dplyr::mutate(test_res = purrr::map(data, region_test)) %>%
#     tidyr::unnest(test_res) %>%
#     dplyr::ungroup()
#   
#   ## separo coord
#   res <- res %>%
#     tidyr::separate(id, c("seqnames", "start", "end"),
#                     "_", remove = FALSE, convert = TRUE)
#   
#   ## calcolo mediane per ogni gruppo
#   medians <- filter %>%
#     dplyr::group_by(id, !!rlang::sym(groupcol)) %>%
#     dplyr::summarise(median = stats::median(.data[[statistic]]),
#                      .groups = "drop") %>%
#     tidyr::pivot_wider(names_from = !!rlang::sym(groupcol),
#                        values_from = median)
#   
#   colnames(medians)[-1] <- paste0("median_", colnames(medians)[-1])
#   
#   ## merge
#   res <- res %>%
#     dplyr::inner_join(medians, by = "id") %>%
#     dplyr::filter(!is.na(p.value)) %>%
#     dplyr::mutate(statistic = statistic)
#   
#   ## reduce opzionale come prima
#   if (reduce) {
#     varcols <- colnames(res)[-c(1:4)]
#     res <- res %>%
#       plyranges::as_granges() %>%
#       dplyr::group_by(!!!rlang::syms(varcols)) %>%
#       plyranges::reduce_ranges() %>%
#       as_tibble() %>%
