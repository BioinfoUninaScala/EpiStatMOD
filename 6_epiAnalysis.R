                      ...
                      get.cPos=FALSE)
{
  ...
  if((!mode=="CG") & (stranded == FALSE))
  {
    ...
    Mapi = foreach::foreach(a = df_split, 
                            b = regs, 
                            .verbose = TRUE, 
                            .export = c('epiallele_analyse_Block', 'epiallele_analyse', 'get_cPos', 'get_epiMatrix', 'get_out', 'extract_matrix')
                            ) %dopar% {
      block = epiallele_analyse_Block(df = a, aln_c = b, threshold, bisu.Thresh, stranded, mode, remove.Amb, retain.reads, get.cPos, myfuns)
    }
    ...
    data.table::fwrite(epi, epiFile, sep = "\t")
    
    logFile=paste(pathDir,paste(aname,"log.txt", sep="_"),sep="/")
    data.table::fwrite(log, logFile, sep = "\t", col.names = FALSE)
...
{
  if(mode=="bisu"){
    data=lapply(pos, function(x) {y = XVector::subseq(alignObj, x, x); return(base::unname(as.vector(y)))})
    ...
    data=lapply(pos, function(x) {y = XVector::subseq(alignObj, x, x+1); return(base::unname(as.vector(y)))})
    data=as.data.frame(do.call(cbind,data))
    if(strand == "plus")
    {
...
{
  data_mode=extract_matrix(alignObj = alignObj, pos = c.Mode, strand, mode)
  if(remove.Amb==TRUE)
  {
...
epiallele_analyse=function(align,
      ...
      length(reads) < threshold)
  {
             ...
             bin@strand@values == "+")
    {
      ...
      data_plus=tidyr::as_tibble(get_epiMatrix(alignObj = align_plus, bisu.Thresh, remove.Amb, c.Mode = cPos[[1]], c.NotMode = cPos[[2]], "plus", retain.reads, mode))
      ...
      data_minus=tidyr::as_tibble(get_epiMatrix(alignObj = align_minus, 
                                                ...
                                                c.Mode = cPos[[3]], c.NotMode = cPos[[4]],
                                                strand =  "minus", retain.reads, mode))
