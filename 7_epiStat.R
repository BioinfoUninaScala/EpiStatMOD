...
epiStat <- function(sample_list, metadata, colgroups, colsamples,
  ...
  res = foreach::foreach(a = regs, b = filt_samples,
                         ...
                         .export = c('allStat', 'oneStat', 'getEpimatrix')) %dopar% {
  ...
  if(reduce == TRUE){
    ...
    gr = gr %>% dplyr::group_by(F.statistics, p.value, num_EpiSpecies, replicates) %>%
      plyranges::reduce_ranges()
    # Convert again into a tibble
    prova = tidyr::as_tibble(gr)
    # Create the id column
    prova= prova %>% tidyr::separate(replicates, groups, sep = "_") %>%
